---
title: 'CLI Reference'
description: 'Command line interface for Seawatts'
---

## Installation

Install the Seawatts CLI globally using your preferred package manager:
...
```bash yarn
yarn global add @seawatts/cli
```

```bash pnpm
pnpm add -g @seawatts/cli
```

```bash bun
bun add -g @seawatts/cli
```
...
```bash
https://seawatts.sh/wh_your_webhook_id
```
...
seawatts listen --client-id dev-1

# With debug logging
seawatts listen --debug

# Redirect to remote URL
seawatts --redirect https://api.example.com
```
...
# Using bunx
bunx @seawatts/cli init

# Using pnpm
pnpm dlx @seawatts/cli init

# Using deno
deno run --allow-net --allow-read --allow-write npm:@seawatts/cli init
```
...
### Configuration File
Create an `seawatts.config.yaml` in your project root:
...
seawatts listen --ping false

# Check specific port
seawatts listen --ping 3001

# Check specific URL
seawatts listen --ping https://api.example.com/health
```

## Authentication

Authentication data is stored locally at `~/.seawatts/auth-storage.json`:
...
seawatts listen --webhook-id your_webhook_id
```

### Team Development
```bash
# Developer 1
seawatts listen --client-id dev1 --webhook-id team_webhook_id

# Developer 2
seawatts listen --client-id dev2 --webhook-id team_webhook_id
```

### Production Redirect
```bash
# Redirect to production API
seawatts --redirect https://api.production.com --webhook-id prod_webhook_id
```

### Custom Health Checks
```bash
# Check specific health endpoint
seawatts listen --ping https://localhost:3000/health
```
...
2. **Authentication Problems**
   - Clear auth data: `rm ~/.seawatts/auth-storage.json`
   - Re-run initialization: `npx @seawatts/cli init`

3. **Debug Mode**
   - Enable debug logging: `seawatts listen --debug`
   - Check the debug panel for detailed information

## Support

- [Documentation](https://docs.seawatts.sh)
- [GitHub Issues](https://github.com/seawatts-sh/seawatts/issues)
- [Discord Community](https://discord.gg/seawatts)