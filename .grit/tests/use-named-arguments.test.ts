...
async function testFiveParams(a: string, b: number, c: boolean, d: string, e: number) {
  return `${a}${b}${c}${d}${e}`;
}
...
const funcExpr = (a: string, b: number, c: boolean, d: string): string => `${a}${b}${c}${d}`;

// ✅ Should trigger: function with return type and 4 parameters
function testWithReturnType(a: string, b: number, c: boolean, d: string): string {
  return `${a}${b}${c}${d}`;
}
