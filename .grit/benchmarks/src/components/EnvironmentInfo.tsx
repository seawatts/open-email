  ...
  return (
    <div className='rounded-lg border bg-card p-4 text-sm shadow-sm'>
      ...
      <dl className='grid grid-cols-[auto_1fr] gap-x-4 gap-y-2'>
        ...
        <dd className='text-muted-foreground'>{new Date(data.timestamp).toLocaleString()}</dd>
        ...
        <dd className='text-muted-foreground'>{data.environment.commit.substring(0, 8)}</dd>
      </dl>
    </div>
  );
}
